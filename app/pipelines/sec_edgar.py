...
def _run_collection(tickers: list[str], filing_types: list[str], limit_per_type: int = 1) -> dict[str, int]:
...
def collect_for_tickers(tickers: list[str], filing_types: list[str], limit_per_type: int = 1) -> dict[str, int]:
    tickers = [t.upper().strip() for t in tickers]
    return _run_collection(tickers=tickers, filing_types=filing_types, limit_per_type=limit_per_type)


def main() -> None:
    _run_collection(tickers=DEFAULT_TARGET_TICKERS, filing_types=DEFAULT_FILING_TYPES, limit_per_type=1)
